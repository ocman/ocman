...
func TestArchiveWorkflowVersionHidesDefinitionButKeepsRuns(t *testing.T) {
	...
	if err := db.InsertWorkflowRun(t.Context(), WorkflowRun{ID: "run-1", WorkflowID: version.WorkflowID, VersionID: version.ID, State: "successful", CreatedAt: 1, UpdatedAt: 1}); err != nil {
		t.Fatal(err)
	}
	if err := db.ArchiveWorkflowVersion(t.Context(), version.ID, 2); err != nil {
		t.Fatal(err)
	}
	versions, err := db.ListWorkflowVersions(t.Context())
	if err != nil || len(versions) != 0 {
		t.Fatalf("archived workflow still listed: %#v, %v", versions, err)
	}
	runs, err := db.ListWorkflowRuns(t.Context())
	if err != nil || len(runs) != 1 {
		t.Fatalf("archiving removed run history: %#v, %v", runs, err)
	}
	if _, err := db.GetActiveWorkflowVersion(t.Context(), version.WorkflowID); !errors.Is(err, sql.ErrNoRows) {
...
func TestResolveWorkflowAttempt(t *testing.T) {
	...
	if err := db.InsertWorkflowRun(t.Context(), WorkflowRun{ID: "run-1", WorkflowID: version.WorkflowID, VersionID: version.ID, State: "active", CreatedAt: 1, UpdatedAt: 1, Nodes: []WorkflowNodeRun{{NodeID: "approval", State: "ready"}}}); err != nil {
		t.Fatal(err)
	}
	...
	if err := db.ResolveWorkflowAttempt(t.Context(), "run-1", attemptID, "successful", 2); err != nil {
		t.Fatal(err)
	}
	run, err = db.GetWorkflowRun(t.Context(), "run-1")
	if err != nil {
	...
	if err := db.ResolveWorkflowAttempt(t.Context(), "run-1", attemptID, "failed", 3); err == nil || !strings.Contains(err.Error(), "unknown attempt") {
...
func TestMarkWorkflowAttemptUnknownIgnoresSettledAttempt(t *testing.T) {
	db := openTestStateDB(t)
	if _, err := db.InsertWorkflowVersion(t.Context(), WorkflowVersion{
		ID: "version-1", WorkflowID: "workflow-1", Name: "Workflow", MetadataVersion: "1",
	...
	if err := db.CompleteWorkflowAgentNode(t.Context(), "run-1", "review", attemptID,
		WorkflowAgentResult{Successful: true, SessionState: "done", OutputsJSON: "{}"}, 2); err != nil {
		t.Fatal(err)
	}
	if settled, err := db.GetWorkflowRun(t.Context(), "run-1"); err != nil {
		t.Fatal(err)
	...
	if err := db.MarkWorkflowAttemptUnknown(t.Context(), "run-1", "review", attemptID, "unreachable", 3); err != nil {
		t.Fatal(err)
	}

	after, err := db.GetWorkflowRun(t.Context(), "run-1")
	if err != nil {
		t.Fatal(err)
	}
...
func TestWorkflowLifecyclePersistence(t *testing.T) {
	...
	if _, err := db.DeactivateWorkflowVersion(t.Context(), v2.ID, 4); err != nil {
		t.Fatal(err)
	}
	if _, err := db.ActivateWorkflowVersion(t.Context(), v2.ID, 5); err != nil {
		t.Fatal(err)
	}
	active, err := db.GetActiveWorkflowVersion(t.Context(), v2.WorkflowID)
	...
	if started, err := db.StartWorkflowCommand(t.Context(), run.ID, "build", []WorkflowResourceRequest{{Pool: "", Units: 1, Capacity: 2}}, nil, 11); err != nil || !started {
		t.Fatalf("start command: %v, %v", started, err)
	}
	if err := db.CompleteWorkflowCommand(t.Context(), run.ID, "build", WorkflowCommandResult{State: "successful", ExitCode: 0, Stdout: `{"built":true}`, OutputsJSON: `{"built":true}`}, 12); err != nil {
		t.Fatal(err)
	}
	persisted, err := db.GetWorkflowRun(t.Context(), run.ID)
	...
	if claimed, err := db.ClaimWorkflowAgentAttempt(t.Context(), run.ID, "review", attemptID, "host", "/repo", []WorkflowResourceRequest{{Pool: "", Units: 1, Capacity: 2}}, nil, 13); err != nil || !claimed {
		t.Fatalf("claim agent: %v, %v", claimed, err)
	}
	if err := db.AttachWorkflowAgentSession(t.Context(), run.ID, "review", attemptID, "opencode", "session-1", "busy", 14); err != nil {
		t.Fatal(err)
	}
	if err := db.SetWorkflowAgentSessionState(t.Context(), run.ID, "review", attemptID, "idle", "", 15); err != nil {
		t.Fatal(err)
	}
	if err := db.CompleteWorkflowAgentNode(t.Context(), run.ID, "review", attemptID, WorkflowAgentResult{Successful: true, SessionState: "idle", Output: `{"approved":true}`, OutputsJSON: `{"approved":true}`}, 16); err != nil {
		t.Fatal(err)
	}
	persisted, err = db.GetWorkflowRun(t.Context(), run.ID)
	...
	if err := db.InsertWorkflowRun(t.Context(), child); err != nil {
		t.Fatal(err)
	}
	children, err := db.ListWorkflowChildRuns(t.Context(), run.ID)
	...
	for _, id := range []string{"run-cancel", "run-fail"} {
		...
		if err := db.InsertWorkflowRun(t.Context(), candidate); err != nil {
			t.Fatal(err)
		}
		if started, err := db.StartWorkflowCommand(t.Context(), id, "build", []WorkflowResourceRequest{{Pool: "", Units: 1, Capacity: 2}}, nil, 31); err != nil || !started {
			t.Fatalf("start %s: %v, %v", id, started, err)
		}
	}
	if err := db.SetWorkflowRunState(t.Context(), "run-cancel", "active", "paused", 32); err != nil {
		t.Fatal(err)
	}
	if err := db.SetWorkflowRunState(t.Context(), "run-cancel", "paused", "active", 33); err != nil {
		t.Fatal(err)
	}
	if err := db.SetWorkflowRunState(t.Context(), "run-cancel", "active", "canceled", 34); err != nil {
		t.Fatal(err)
	}
	if err := db.FailWorkflowRun(t.Context(), "run-fail", "budget exhausted", 35); err != nil {
		t.Fatal(err)
	}
	for _, want := range []struct{ id, state string }{{"run-cancel", "canceled"}, {"run-fail", "failed"}} {
		got, err := db.GetWorkflowRun(t.Context(), want.id)
...
func seedResourceRun(t *testing.T) *DB {
	...
	if err := db.InsertWorkflowRun(t.Context(), WorkflowRun{ID: "run-1", WorkflowID: version.WorkflowID, VersionID: version.ID, State: "active", CreatedAt: 1, UpdatedAt: 1, Nodes: []WorkflowNodeRun{{NodeID: "one", State: "ready"}, {NodeID: "two", State: "ready"}}}); err != nil {
		t.Fatal(err)
	}
	return db
}
...
func TestStartWorkflowCommandAtomicAcquireAllOrNothing(t *testing.T) {
	...
	started, err := db.StartWorkflowCommand(t.Context(), "run-1", "one", req, nil, 10)
	if err != nil || !started {
	...
	started, err = db.StartWorkflowCommand(t.Context(), "run-1", "two", req2, nil, 11)
	if err != nil {
	...
	leases, err := db.ListWorkflowResourceLeases(t.Context(), "run-1")
	if err != nil {
		t.Fatal(err)
	}
...
func TestResourceLeasesSurviveReopen(t *testing.T) {
	...
	if err := db.InsertWorkflowRun(t.Context(), WorkflowRun{ID: "r", WorkflowID: version.WorkflowID, VersionID: version.ID, State: "active", CreatedAt: 1, UpdatedAt: 1, Nodes: []WorkflowNodeRun{{NodeID: "one", State: "ready"}}}); err != nil {
		t.Fatal(err)
	}
	if started, err := db.StartWorkflowCommand(t.Context(), "r", "one", []WorkflowResourceRequest{{Pool: "", Units: 1, Capacity: 1}, {Pool: "compiler", Units: 1, Capacity: 1}}, nil, 5); err != nil || !started {
	...
	leases, err := reopened.ListWorkflowResourceLeases(t.Context(), "r")
	if err != nil {
		t.Fatal(err)
	}
	if len(leases) != 2 {
