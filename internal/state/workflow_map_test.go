...
func TestWorkflowMapItemCreationIsAtomicAndIdempotent(t *testing.T) {
	...
	created, err := db.CreateWorkflowMapItem(item, child)
	if err != nil || !created {
	...
	created, err = db.CreateWorkflowMapItem(item, duplicate)
	if err != nil || created {
	...
	if err := db.SetWorkflowMapItemState(item.RunID, item.MapNode, item.ItemKey, "successful"); err != nil {
		t.Fatal(err)
	}
	items, err := db.ListWorkflowMapItems(item.RunID, item.MapNode)
	...
	if created, err := db.CreateWorkflowMapItem(badItem, orphan); err == nil || created {
		t.Fatalf("invalid map item creation: %v, %v", created, err)
	}
	if _, err := db.GetWorkflowRun(orphan.ID); err == nil {
