...
func TestWorkflowArtifactDedupKeepsIndependentMetadata(t *testing.T) {
	...
	if err := db.InsertWorkflowArtifact(a1); err != nil {
		t.Fatalf("insert a1: %v", err)
	}
	if err := db.InsertWorkflowArtifact(a2); err != nil {
		t.Fatalf("insert a2: %v", err)
	}
	list, err := db.ListWorkflowArtifacts("run")
	if err != nil {
		t.Fatalf("list: %v", err)
	}
	if len(list) != 2 {
	...
	hashes, err := db.ExpiredWorkflowArtifactHashes(300)
	if err != nil {
	...
	hashes, err = db.ExpiredWorkflowArtifactHashes(6000)
	if err != nil {
...
func TestWorkflowArtifactNeverExpiresWhenExpiresZero(t *testing.T) {
	db := artifactTestDB(t)
	if err := db.InsertWorkflowArtifact(WorkflowArtifact{ID: "keep", RunID: "run", NodeID: "n", AttemptID: 1, Name: "out", Kind: "text", ContentHash: "keephash", SizeBytes: 1, CreatedAt: 0, ExpiresAt: 0}); err != nil {
		t.Fatalf("insert: %v", err)
	}
	got, err := db.GetWorkflowArtifact("keep")
	if err != nil {
		t.Fatalf("get: %v", err)
	}
	if got.ExpiresAt != 0 {
	...
	hashes, err := db.ExpiredWorkflowArtifactHashes(1 << 62)
	if err != nil {
...
func TestMarkWorkflowArtifactPayloadDeletedPreservesMetadata(t *testing.T) {
	db := artifactTestDB(t)
	if err := db.InsertWorkflowArtifact(WorkflowArtifact{ID: "gone", RunID: "run", NodeID: "n", AttemptID: 1, Name: "big", Kind: "diagnostics", ContentHash: "h", SizeBytes: 42, CreatedAt: 10, ExpiresAt: 20}); err != nil {
		t.Fatalf("insert: %v", err)
	}
	if err := db.MarkWorkflowArtifactPayloadDeleted("h"); err != nil {
		t.Fatalf("mark deleted: %v", err)
	}
	got, err := db.GetWorkflowArtifact("gone")
	if err != nil {
		t.Fatalf("get: %v", err)
	}
