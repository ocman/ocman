...
func TestMigrateV80RepairsMissingWebhookInbox(t *testing.T) {
	db, err := sql.Open("sqlite", ":memory:")
	if err != nil {
		t.Fatal(err)
	}
	defer db.Close()
	if _, err := db.Exec(`CREATE TABLE schema_version (version INTEGER PRIMARY KEY, applied_at INTEGER NOT NULL); INSERT INTO schema_version VALUES (79, 0)`); err != nil {
		t.Fatal(err)
	}
	...
	var count int
	if err := db.QueryRow(`SELECT count(*) FROM sqlite_master WHERE type = 'table' AND name = 'webhook_inbox'`).Scan(&count); err != nil {
		t.Fatal(err)
	}
	if count != 1 {
...
func TestMigrateV46DropsChildSessionsAndPreservesState(t *testing.T) {
	...
	if err := db.QueryRow(`SELECT count(*) FROM queued_message WHERE id = 'keep-collision'`).Scan(&count); err != nil || count != 1 {
		t.Fatalf("preserved user message matching legacy fragments = %d, %v", count, err)
	}
}
