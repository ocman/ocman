...
func acquireWorkflowResources(ctx context.Context, tx *sql.Tx, runID, nodeID string, attemptID int64, requests []WorkflowResourceRequest, now int64) (bool, error) {
	for _, req := range requests {
		if req.Units <= 0 {
			continue
		}
		var held int
		if err := tx.QueryRowContext(ctx, `SELECT COALESCE(SUM(units), 0) FROM workflow_resource_lease WHERE run_id = ? AND pool = ? AND node_id != ?`, runID, req.Pool, nodeID).Scan(&held); err != nil {
	...
	for _, req := range requests {
		...
		if _, err := tx.ExecContext(ctx, `INSERT OR IGNORE INTO workflow_resource_lease (run_id, node_id, attempt_id, pool, units, acquired_at) VALUES (?, ?, ?, ?, ?, ?)`, runID, nodeID, attemptID, req.Pool, req.Units, now); err != nil {
...
func releaseWorkflowResources(ctx context.Context, exec workflowRunExecer, runID, nodeID string) error {
	if _, err := exec.ExecContext(ctx, `DELETE FROM workflow_resource_lease WHERE run_id = ? AND node_id = ?`, runID, nodeID); err != nil {
		return fmt.Errorf("releasing resources: %w", err)
	}
	if err := releaseWorkflowWorkspaceLease(ctx, exec, runID, nodeID); err != nil {
		return err
	}
	return nil
}
...
func deleteAllWorkflowLeases(ctx context.Context, exec workflowRunExecer, runID string) error {
	if _, err := exec.ExecContext(ctx, `DELETE FROM workflow_resource_lease WHERE run_id = ?`, runID); err != nil {
		return fmt.Errorf("releasing resources: %w", err)
	}
	if _, err := exec.ExecContext(ctx, `DELETE FROM workflow_workspace_lease WHERE run_id = ?`, runID); err != nil {
...
func (d *DB) ListWorkflowResourceLeases(ctx context.Context, runID string) ([]WorkflowResourceLease, error) {
	rows, err := d.db.QueryContext(ctx, `SELECT run_id, node_id, attempt_id, pool, units, acquired_at FROM workflow_resource_lease WHERE run_id = ? ORDER BY pool, node_id`, runID)
	if err != nil {
...
func (d *DB) InsertWorkflowVersion(ctx context.Context, v WorkflowVersion) (WorkflowVersion, error) {
	...
	var revision int
	err = tx.QueryRowContext(ctx, `SELECT current_revision FROM workflow_definition WHERE id = ?`, v.WorkflowID).Scan(&revision)
	switch {
	case err == nil:
		revision++
		if _, err = tx.ExecContext(ctx, `UPDATE workflow_definition SET name = ?, current_revision = ?, updated_at = ? WHERE id = ?`, v.Name, revision, v.CreatedAt, v.WorkflowID); err != nil {
			return WorkflowVersion{}, fmt.Errorf("updating workflow definition: %w", err)
		}
	case errors.Is(err, sql.ErrNoRows):
		revision = 1
		if _, err = tx.ExecContext(ctx, `INSERT INTO workflow_definition (id, name, current_revision, created_at, updated_at) VALUES (?, ?, ?, ?, ?)`, v.WorkflowID, v.Name, revision, v.CreatedAt, v.CreatedAt); err != nil {
	...
	v.Revision = revision
	if _, err = tx.ExecContext(ctx, `INSERT INTO workflow_version (id, workflow_id, name, revision, metadata_version, definition_json, concurrency, retention_days, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`, v.ID, v.WorkflowID, v.Name, v.Revision, v.MetadataVersion, v.DefinitionJSON, v.Concurrency, v.RetentionDays, v.CreatedAt); err != nil {
		return WorkflowVersion{}, fmt.Errorf("inserting workflow version: %w", err)
	}
	for _, node := range v.Nodes {
		if _, err = tx.ExecContext(ctx, `INSERT INTO workflow_version_node (version_id, node_id, name, type, position, subworkflow_id) VALUES (?, ?, ?, ?, ?, ?)`, v.ID, node.ID, node.Name, node.Type, node.Position, node.SubworkflowID); err != nil {
			return WorkflowVersion{}, fmt.Errorf("inserting workflow node: %w", err)
		}
	}
	for _, dep := range v.Dependencies {
		if _, err = tx.ExecContext(ctx, `INSERT INTO workflow_version_dependency (version_id, from_node, to_node) VALUES (?, ?, ?)`, v.ID, dep.From, dep.To); err != nil {
			return WorkflowVersion{}, fmt.Errorf("inserting workflow dependency: %w", err)
		}
	}
	if v.Revision == 1 {
		if _, err = tx.ExecContext(ctx, `UPDATE workflow_definition SET active_version_id = ? WHERE id = ?`, v.ID, v.WorkflowID); err != nil {
...
func (d *DB) GetWorkflowVersion(ctx context.Context, id string) (*WorkflowVersion, error) {
	...
	v.Nodes, err = d.workflowVersionNodes(ctx, id)
	if err != nil {
		return nil, err
	}
	v.Dependencies, err = d.workflowVersionDependencies(ctx, id)
	if err != nil {
		return nil, err
	}
	return &v, nil
...
func (d *DB) ListWorkflowVersions(ctx context.Context) ([]WorkflowVersion, error) {
...
func (d *DB) ArchiveWorkflowVersion(ctx context.Context, id string, now int64) error {
	res, err := d.db.ExecContext(ctx, `UPDATE workflow_definition SET archived_at = ?, active_version_id = NULL, updated_at = ? WHERE id = (SELECT workflow_id FROM workflow_version WHERE id = ?) AND archived_at IS NULL`, now, now, id)
	if err != nil {
...
func (d *DB) workflowVersionNodes(ctx context.Context, id string) ([]WorkflowNode, error) {
	rows, err := d.db.QueryContext(ctx, `SELECT node_id, name, type, position, subworkflow_id FROM workflow_version_node WHERE version_id = ? ORDER BY position`, id)
	if err != nil {
...
func (d *DB) ActivateWorkflowVersion(ctx context.Context, id string, now int64) (*WorkflowVersion, error) {
	tx, err := d.db.BeginTx(ctx, nil)
	if err != nil {
		return nil, fmt.Errorf("activating workflow version: %w", err)
	}
	defer func() { _ = tx.Rollback() }()
	res, err := tx.ExecContext(ctx, `UPDATE workflow_definition SET active_version_id = ?, updated_at = ? WHERE id = (SELECT workflow_id FROM workflow_version WHERE id = ?)`, id, now, id)
	if err != nil {
		return nil, fmt.Errorf("activating workflow version: %w", err)
	}
	changed, err := res.RowsAffected()
	...
	if _, err := tx.ExecContext(ctx, `UPDATE workflow_trigger_state
	...
	return d.GetWorkflowVersion(ctx, id)
}

func (d *DB) DeactivateWorkflowVersion(ctx context.Context, id string, now int64) (*WorkflowVersion, error) {
	res, err := d.db.ExecContext(ctx, `UPDATE workflow_definition SET active_version_id = NULL, updated_at = ? WHERE active_version_id = ?`, now, id)
	if err != nil {
...
func (d *DB) GetActiveWorkflowVersion(ctx context.Context, workflowID string) (*WorkflowVersion, error) {
	var id string
	if err := d.db.QueryRowContext(ctx, `SELECT active_version_id FROM workflow_definition WHERE id = ? AND active_version_id IS NOT NULL`, workflowID).Scan(&id); err != nil {
		return nil, fmt.Errorf("getting active workflow version: %w", err)
	}
	return d.GetWorkflowVersion(ctx, id)
}

func (d *DB) workflowVersionDependencies(ctx context.Context, id string) ([]WorkflowDependency, error) {
	rows, err := d.db.QueryContext(ctx, `SELECT from_node, to_node FROM workflow_version_dependency WHERE version_id = ? ORDER BY rowid`, id)
	if err != nil {
...
func (d *DB) InsertWorkflowRun(ctx context.Context, run WorkflowRun) error {
	tx, err := d.db.BeginTx(ctx, nil)
	if err != nil {
...
func (d *DB) ListWorkflowChildRuns(ctx context.Context, parentRunID string) ([]WorkflowRun, error) {
...
func (d *DB) GetWorkflowRun(ctx context.Context, id string) (*WorkflowRun, error) {
	...
	for i := range run.Nodes {
		run.Nodes[i].Attempts, err = d.workflowAttempts(ctx, id, run.Nodes[i].NodeID)
		if err != nil {
			return nil, err
		}
	}
...
func (d *DB) workflowAttempts(ctx context.Context, runID, nodeID string) ([]WorkflowAttempt, error) {
	rows, err := d.db.QueryContext(ctx, `
...
func (d *DB) StartWorkflowCommand(ctx context.Context, runID, nodeID string, requests []WorkflowResourceRequest, workspace *WorkflowWorkspaceRequest, now int64) (bool, error) {
	tx, err := d.db.BeginTx(ctx, nil)
	if err != nil {
	...
	acquired, err := acquireWorkflowResources(ctx, tx, runID, nodeID, attemptID, requests, now)
	if err != nil {
		return false, err
	}
	if !acquired {
	...
	if workspace != nil {
		if _, ok, err := acquireWorkflowWorkspaceLease(ctx, tx, runID, nodeID, attemptID, *workspace, now); err != nil {
			return false, err
		} else if !ok {
			return false, nil
		}
	}
	if _, err = tx.ExecContext(ctx, `UPDATE workflow_node_attempt SET state = 'running', started_at = ? WHERE id = ?`, now, attemptID); err != nil {
...
func (d *DB) CompleteWorkflowCommand(ctx context.Context, runID, nodeID string, result WorkflowCommandResult, now int64) error {
	tx, err := d.db.BeginTx(ctx, nil)
	if err != nil {
	...
	if err = tx.QueryRowContext(ctx, `SELECT state FROM workflow_node_attempt WHERE run_id = ? AND node_id = ? AND state = 'running'`, runID, nodeID).Scan(&attemptState); err != nil {
	...
	if _, err = tx.ExecContext(ctx, `UPDATE workflow_node_run SET state = ?, completed_at = ? WHERE run_id = ? AND node_id = ? AND state = 'ready'`, nodeState, now, runID, nodeID); err != nil {
...
func (d *DB) ClaimWorkflowAgentAttempt(ctx context.Context, runID, nodeID string, attemptID int64, affinity, directory string, requests []WorkflowResourceRequest, workspace *WorkflowWorkspaceRequest, now int64) (bool, error) {
	tx, err := d.db.BeginTx(ctx, nil)
	if err != nil {
		return false, err
	}
	...
	err = tx.QueryRowContext(ctx, `SELECT state FROM workflow_node_attempt WHERE id = ? AND run_id = ? AND node_id = ?`, attemptID, runID, nodeID).Scan(&attemptState)
	...
	res, err := tx.ExecContext(ctx, `UPDATE workflow_node_attempt SET state = 'starting', affinity = ?, directory = ? WHERE id = ? AND run_id = ? AND node_id = ? AND state = 'waiting'`, affinity, directory, attemptID, runID, nodeID)
	if err != nil {
	...
	if _, err = tx.ExecContext(ctx, `UPDATE workflow_node_run SET state = 'running' WHERE run_id = ? AND node_id = ? AND state = 'ready'`, runID, nodeID); err != nil {
		return false, err
	}
	if _, err = tx.ExecContext(ctx, `UPDATE workflow_run SET updated_at = ? WHERE id = ?`, now, runID); err != nil {
		return false, err
	}
	return true, tx.Commit()
}

func (d *DB) AttachWorkflowAgentSession(ctx context.Context, runID, nodeID string, attemptID int64, platform, sessionID, sessionState string, now int64) error {
	res, err := d.db.ExecContext(ctx, `UPDATE workflow_node_attempt SET state = 'running', platform = ?, session_id = ?, session_state = ? WHERE id = ? AND run_id = ? AND node_id = ? AND state = 'starting'`, platform, sessionID, sessionState, attemptID, runID, nodeID)
	if err != nil {
	...
	_, err = d.db.ExecContext(ctx, `UPDATE workflow_run SET updated_at = ? WHERE id = ?`, now, runID)
	return err
}

func (d *DB) SetWorkflowAgentSessionState(ctx context.Context, runID, nodeID string, attemptID int64, sessionState, attemptError string, now int64) error {
	_, err := d.db.ExecContext(ctx, `UPDATE workflow_node_attempt SET session_state = ?, error = ?, completed_at = CASE WHEN ? = 'canceled' THEN ? ELSE completed_at END WHERE id = ? AND run_id = ? AND node_id = ?`, sessionState, attemptError, sessionState, now, attemptID, runID, nodeID)
	if err != nil {
...
func (d *DB) CompleteWorkflowAgentNode(ctx context.Context, runID, nodeID string, attemptID int64, result WorkflowAgentResult, now int64) error {
	tx, err := d.db.BeginTx(ctx, nil)
	if err != nil {
	...
	if _, err = tx.ExecContext(ctx, `UPDATE workflow_node_run SET state = ?, completed_at = ? WHERE run_id = ? AND node_id = ? AND state = 'running'`, nodeState, now, runID, nodeID); err != nil {
		return fmt.Errorf("completing workflow agent node: %w", err)
	}
	if _, err = tx.ExecContext(ctx, `UPDATE workflow_node_attempt SET state = ?, session_state = ?, stdout = ?, outputs_json = ?, error = ?, completed_at = ? WHERE id = ? AND state IN ('starting', 'running')`, attemptState, result.SessionState, result.Output, result.OutputsJSON, result.Error, now, attemptID); err != nil {
...
func completeWorkflowNodeTx(ctx context.Context, tx *sql.Tx, runID string, now int64) error {
	...
	for {
		res, err := tx.ExecContext(ctx, `UPDATE workflow_node_run SET state = 'skipped', completed_at = ? WHERE run_id = ? AND state = 'pending' AND EXISTS (
	...
	var remaining int
	if err := tx.QueryRowContext(ctx, `SELECT count(*) FROM workflow_node_run WHERE run_id = ? AND state IN ('pending', 'ready', 'running')`, runID).Scan(&remaining); err != nil {
		return fmt.Errorf("counting workflow nodes: %w", err)
	}
	if remaining == 0 {
		var failed int
		if err := tx.QueryRowContext(ctx, `SELECT count(*) FROM workflow_node_run WHERE run_id = ? AND state = 'failed'`, runID).Scan(&failed); err != nil {
			return err
		}
		state := "successful"
		if failed > 0 {
		...
		if _, err := tx.ExecContext(ctx, `UPDATE workflow_run SET state = ?, updated_at = ?, completed_at = ? WHERE id = ?`, state, now, now, runID); err != nil {
			return fmt.Errorf("settling workflow run: %w", err)
		}
		return tx.Commit()
	}
	var versionID string
	if err := tx.QueryRowContext(ctx, `SELECT version_id FROM workflow_run WHERE id = ?`, runID).Scan(&versionID); err != nil {
		return err
	}
	...
	for _, id := range ready {
		if _, err := tx.ExecContext(ctx, `UPDATE workflow_node_run SET state = 'ready', ready_at = ? WHERE run_id = ? AND node_id = ?`, now, runID, id); err != nil {
			return err
		}
		if _, err := tx.ExecContext(ctx, `INSERT INTO workflow_node_attempt (run_id, node_id, seq, state, started_at) VALUES (?, ?, 1, 'waiting', ?)`, runID, id, now); err != nil {
			return err
		}
	}
	if _, err := tx.ExecContext(ctx, `UPDATE workflow_run SET updated_at = ? WHERE id = ?`, now, runID); err != nil {
...
func (d *DB) ApproveWorkflowNode(ctx context.Context, runID, nodeID string, now int64) error {
	tx, err := d.db.BeginTx(ctx, nil)
	if err != nil {
	...
	if err = tx.QueryRowContext(ctx, `SELECT state, version_id FROM workflow_run WHERE id = ?`, runID).Scan(&state, &versionID); err != nil {
	...
	if _, err = tx.ExecContext(ctx, `UPDATE workflow_node_run SET state = 'successful', completed_at = ? WHERE run_id = ? AND node_id = ?`, now, runID, nodeID); err != nil {
		return fmt.Errorf("completing workflow node: %w", err)
	}
	if _, err = tx.ExecContext(ctx, `UPDATE workflow_node_attempt SET state = 'successful', completed_at = ? WHERE run_id = ? AND node_id = ? AND state = 'waiting'`, now, runID, nodeID); err != nil {
		return fmt.Errorf("completing workflow attempt: %w", err)
	}
	var remaining int
	if err = tx.QueryRowContext(ctx, `SELECT count(*) FROM workflow_node_run WHERE run_id = ? AND state != 'successful'`, runID).Scan(&remaining); err != nil {
		return fmt.Errorf("counting workflow nodes: %w", err)
	}
	if remaining == 0 {
		if _, err = tx.ExecContext(ctx, `UPDATE workflow_run SET state = 'successful', updated_at = ?, completed_at = ? WHERE id = ?`, now, now, runID); err != nil {
		...
		for _, node := range ready {
			if _, err = tx.ExecContext(ctx, `UPDATE workflow_node_run SET state = 'ready', ready_at = ? WHERE run_id = ? AND node_id = ?`, now, runID, node.id); err != nil {
				return fmt.Errorf("readying workflow node: %w", err)
			}
			if _, err = tx.ExecContext(ctx, `INSERT INTO workflow_node_attempt (run_id, node_id, seq, state, started_at) VALUES (?, ?, 1, 'waiting', ?)`, runID, node.id, now); err != nil {
...
func (d *DB) FailWorkflowRun(ctx context.Context, id, reason string, now int64) error {
	tx, err := d.db.BeginTx(ctx, nil)
	if err != nil {
	...
	defer func() { _ = tx.Rollback() }()
	var runState string
	if err = tx.QueryRowContext(ctx, `SELECT state FROM workflow_run WHERE id = ?`, id).Scan(&runState); err != nil {
		return fmt.Errorf("getting workflow run: %w", err)
	}
	if runState != "active" && runState != "paused" {
		return nil
	}
	if _, err = tx.ExecContext(ctx, `UPDATE workflow_node_attempt SET state = 'canceled', error = CASE WHEN error = '' THEN ? ELSE error END, session_state = CASE WHEN session_id != '' THEN 'canceled' ELSE session_state END, completed_at = ? WHERE run_id = ? AND state IN ('waiting', 'starting', 'running')`, reason, now, id); err != nil {
		return fmt.Errorf("canceling attempts on budget fail: %w", err)
	}
	if _, err = tx.ExecContext(ctx, `UPDATE workflow_node_run SET state = 'skipped', completed_at = ? WHERE run_id = ? AND state = 'pending'`, now, id); err != nil {
		return fmt.Errorf("skipping unreachable nodes on budget fail: %w", err)
	}
	if _, err = tx.ExecContext(ctx, `UPDATE workflow_node_run SET state = 'canceled', completed_at = ? WHERE run_id = ? AND state IN ('ready', 'running')`, now, id); err != nil {
		return fmt.Errorf("canceling nodes on budget fail: %w", err)
	}
	if err = deleteAllWorkflowLeases(ctx, tx, id); err != nil {
		return err
	}
	if _, err = tx.ExecContext(ctx, `UPDATE workflow_run SET state = 'failed', updated_at = ?, completed_at = ? WHERE id = ?`, now, now, id); err != nil {
		return fmt.Errorf("failing workflow run: %w", err)
	}
	return tx.Commit()
}

func (d *DB) SetWorkflowRunState(ctx context.Context, id, from, to string, now int64) error {
	tx, err := d.db.BeginTx(ctx, nil)
	if err != nil {
	...
	if to == "canceled" {
		...
		if _, err = tx.ExecContext(ctx, `UPDATE workflow_node_attempt SET state = 'canceled', session_state = CASE WHEN session_id != '' THEN 'canceled' ELSE session_state END, completed_at = ? WHERE run_id = ? AND state IN ('waiting', 'starting', 'running')`, now, id); err != nil {
	...
	res, err := tx.ExecContext(ctx, `UPDATE workflow_run SET state = ?, updated_at = ?, completed_at = ? WHERE id = ? AND state = ?`, to, now, completed, id, from)
	if err != nil {
...
func (d *DB) ResolveWorkflowAttempt(ctx context.Context, runID string, attemptID int64, resolution string, now int64) error {
	return d.ResolveWorkflowAttemptBy(ctx, runID, attemptID, resolution, "user", now)
}
...
func (d *DB) ResolveWorkflowAttemptBy(ctx context.Context, runID string, attemptID int64, resolution, resolvedBy string, now int64) error {
	tx, err := d.db.BeginTx(ctx, nil)
	if err != nil {
	...
	if resolution == "retry" {
		if _, err = tx.ExecContext(ctx, `UPDATE workflow_node_attempt SET state = 'failed', completed_at = ?, resolved_at = ?, resolved_by = ? WHERE id = ?`, now, now, resolvedBy, attemptID); err != nil {
		...
		var nextSeq int
		if err = tx.QueryRowContext(ctx, `SELECT COALESCE(max(seq), 0) + 1 FROM workflow_node_attempt WHERE run_id = ? AND node_id = ?`, runID, nodeID).Scan(&nextSeq); err != nil {
			return fmt.Errorf("computing retry attempt seq: %w", err)
		}
		if _, err = tx.ExecContext(ctx, `INSERT INTO workflow_node_attempt (run_id, node_id, seq, state, started_at) VALUES (?, ?, ?, 'waiting', ?)`, runID, nodeID, nextSeq, now); err != nil {
			return fmt.Errorf("creating retry attempt: %w", err)
		}
		if _, err = tx.ExecContext(ctx, `UPDATE workflow_node_run SET state = 'ready', ready_at = ?, completed_at = NULL WHERE run_id = ? AND node_id = ?`, now, runID, nodeID); err != nil {
			return fmt.Errorf("readying node for retry: %w", err)
		}
		if _, err = tx.ExecContext(ctx, `UPDATE workflow_run SET state = 'active', updated_at = ? WHERE id = ?`, now, runID); err != nil {
	...
	if _, err = tx.ExecContext(ctx, `UPDATE workflow_node_attempt SET state = ?, completed_at = ?, resolved_at = ?, resolved_by = ? WHERE id = ?`, resolution, now, now, resolvedBy, attemptID); err != nil {
		return fmt.Errorf("updating workflow attempt: %w", err)
	}
	if _, err = tx.ExecContext(ctx, `UPDATE workflow_node_run SET state = ?, completed_at = ? WHERE run_id = ? AND node_id = ?`, resolution, now, runID, nodeID); err != nil {
	...
	if resolution == "failed" {
		if _, err = tx.ExecContext(ctx, `UPDATE workflow_run SET state = 'failed', updated_at = ?, completed_at = ? WHERE id = ?`, now, now, runID); err != nil {
			return fmt.Errorf("failing workflow run: %w", err)
		}
	} else {
		var remaining int
...
func (d *DB) MarkWorkflowAttemptUnknown(ctx context.Context, runID, nodeID string, attemptID int64, reason string, now int64) error {
	tx, err := d.db.BeginTx(ctx, nil)
	if err != nil {
		return fmt.Errorf("beginning unknown attempt transition: %w", err)
	}
	defer func() { _ = tx.Rollback() }()
	res, err := tx.ExecContext(ctx, `UPDATE workflow_node_attempt SET state = 'unknown', error = ?, completed_at = ? WHERE id = ? AND state IN ('starting', 'running')`, reason, now, attemptID)
	if err != nil {
	...
	if _, err = tx.ExecContext(ctx, `UPDATE workflow_node_run SET state = 'unknown' WHERE run_id = ? AND node_id = ?`, runID, nodeID); err != nil {
		return fmt.Errorf("marking node unknown: %w", err)
	}
	if err = releaseWorkflowResources(ctx, tx, runID, nodeID); err != nil {
		return err
	}
	if _, err = tx.ExecContext(ctx, `UPDATE workflow_run SET state = 'paused', updated_at = ? WHERE id = ? AND state = 'active'`, now, runID); err != nil {
...
func (d *DB) RetryWorkflowAttempt(ctx context.Context, runID, nodeID string, attemptID int64, reason, resolvedBy string, now int64) error {
	tx, err := d.db.BeginTx(ctx, nil)
	if err != nil {
		return fmt.Errorf("beginning retry-safe recovery: %w", err)
	}
	defer func() { _ = tx.Rollback() }()
	if _, err = tx.ExecContext(ctx, `UPDATE workflow_node_attempt SET state = 'failed', error = ?, completed_at = ?, resolved_at = ?, resolved_by = ? WHERE id = ? AND state IN ('starting', 'running')`, reason, now, now, resolvedBy, attemptID); err != nil {
