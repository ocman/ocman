...
func formulaRevisionFromState(formula FactoryFormula, revision FactoryFormulaRevision) FormulaRevision {
...
func graphForFormula(definition formulaDefinition, req CreateWorkEpicRequest, provenance func(string) map[string]string) graphPlan {
	...
	plan := graphPlan{CommitMessage: "ocman factory: instantiate " + req.InstantiationID, Nodes: []graphNode{{Key: "epic", Title: req.Goal, Type: "epic", Description: req.Brief, Metadata: epic, MetadataRefs: epicRefs}}}
	for _, node := range definition.Nodes {
		...
		description := renderFormulaText(node.Description, parameters)
		if node.Kind == "agent-work" && req.Brief != "" {
			description = req.Brief
		}
		plan.Nodes = append(plan.Nodes, graphNode{Key: node.Key, Title: renderFormulaText(node.Title, parameters), Type: typeName, Description: description, ParentKey: "epic", Metadata: metadata, MetadataRefs: map[string]string{"ocman.work_epic_id": "epic"}})
	}
	for _, edge := range definition.Edges {
