...
var factoryActions = []factoryAction{
	...
	{name: "submit_proposal", description: "Creates the next immutable issue-graph proposal revision. Define implementation Issues and optional Project Delivery placeholders as nodes. Dependencies point from dependent to blocker and use blocks, on_failure, or merge_gated. A merge_gated edge must target another project's delivery node. Legacy per-node dependsOn keys are blocks. At least one node must be a required implementation and dependencies must be acyclic.", example: `{"action":"submit_proposal","epic_id":"epic-1","manifest_json":"{\"epicId\":\"epic-1\",\"molId\":\"epic-1\",\"project\":\"/repo\",\"nodes\":[{\"key\":\"backend\",\"type\":\"implementation\",\"requirement\":\"required\",\"title\":\"Build backend\",\"description\":\"Add the API.\"},{\"key\":\"frontend\",\"type\":\"implementation\",\"requirement\":\"required\",\"title\":\"Build frontend\",\"description\":\"Add the UI.\"}],\"edges\":[{\"from\":\"frontend\",\"to\":\"backend\",\"type\":\"blocks\"}]}","attempt_id":"fa_1","attempt_token":"fat_1"}`, required: []string{"epic_id", "manifest_json", "attempt_id", "attempt_token"}, optional: []string{"rationale_markdown"}, output: "ProposalRevision", errors: []string{"epic_id is required", "manifest_json is required", "attempt_id and attempt_token are required", "factory action is not permitted", "manifest_json is invalid", "factory epic not found", "factory request failed"}},
	...
	{name: "approve_plan", description: "Approves the exact proposed plan revision and saves the implementation model. Ask the user to confirm the model; recommend Opus/Sol for balanced work or Sonnet/Terra for speed.", example: `{"action":"approve_plan","epic_id":"epic-1","revision":1,"expected_hash":"hash","implementation_model":"openai/gpt-5.6-sol"}`, required: []string{"epic_id", "revision", "expected_hash"}, optional: []string{"feedback", "implementation_model"}, output: "PlanGate", errors: []string{"epic_id is required", "revision is required", "expected_hash is required", "factory request failed"}},
...
func factoryHelp() map[string]any {
	...
	help["reply_cards"] = factoryReplyCards
...
// Denials stay errors; the marker only renders controls for a human to click.
...
func factoryCardMarker(epicID, issueID, action string) string {
	target := "type=factory-epic"
	if epicID != "" {
		if issueID != "" {
			target = "type=factory-issue"
		}
		target += " epic=" + url.PathEscape(epicID)
		if issueID != "" {
			target += " issue=" + url.PathEscape(issueID)
		}
	}
	return "[[ocman:card " + target + " action=" + action + "]]"
}
...
func (t *factoryTools) handleAction(ctx context.Context, req mcplib.CallToolRequest) (*mcplib.CallToolResult, error) {
	...
	case "create":
		...
		var projects []factory.ProjectAdmission
		if raw, present := arguments["projects"]; present {
			encoded, marshalErr := json.Marshal(raw)
			if marshalErr != nil || json.Unmarshal(encoded, &projects) != nil {
	...
	case "approve_plan", "revise_plan", "reject_plan":
		...
		implementationModel, _ := req.RequireString("implementation_model")
		gate, err := t.svc.DecidePlanGate(ctx, id, strings.TrimSuffix(action, "_plan"), factory.PlanGateDecisionRequest{ExpectedRevision: revision, ExpectedHash: hash, Feedback: feedback, ImplementationModel: implementationModel})
		if err != nil {
			return factoryToolError(err), nil
		}
		return toolResultJSON(gate), nil
