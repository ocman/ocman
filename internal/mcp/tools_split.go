...
func splitToSessionTool() mcplib.Tool {
	return mcplib.NewTool("split_to_session",
		mcplib.WithDescription("Launch a new OpenCode session in the same working directory as the parent session, with a context-enriched prompt derived from your intent. Use this when you want to delegate a separable sub-task to a parallel session without leaving the current conversation."),
		...
		mcplib.WithString("intent",
			mcplib.Required(),
			mcplib.Description("Brief description of the sub-task for the new session."),
		),
		mcplib.WithObject("context_options",
			mcplib.Description("Control which context sources are injected into the prompt. All true by default."),
...
func splitToWorktreeTool() mcplib.Tool {
	return mcplib.NewTool("split_to_worktree",
		mcplib.WithDescription("Launch a new OpenCode session in a fresh git worktree, isolated from the current session's working tree. Use this when the sub-task involves file changes that should not interfere with the current session's uncommitted work."),
		mcplib.WithString("session_id",
			mcplib.Required(),
		...
		mcplib.WithString("branch",
			mcplib.Required(),
			mcplib.Description("Branch name for the new worktree (e.g. 'fix/lint-errors')."),
		),
		mcplib.WithString("base_ref",
			mcplib.Description("Base ref for the new branch. Defaults to the repo's default branch (e.g. 'main')."),
		),
		mcplib.WithObject("context_options",
			mcplib.Description("Control which context sources are injected into the prompt. All true by default."),
			mcplib.Properties(map[string]interface{}{
