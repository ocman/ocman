...
// latestAssistantText is the newest assistant answer, sanitized for
// conversation.v1: v1 replies are the plain assistant answer.
func latestAssistantText(messages []db.Message, parts []db.Part) (string, string) {
	if id, text := db.LatestAssistantText(messages, parts); id != "" {
		return id, conversationReplyText(text)
	}
	return "", ""
}
