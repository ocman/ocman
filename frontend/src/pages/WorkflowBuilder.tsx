...
function defaults(type: WorkflowNodeDefinition['type'], id: string): WorkflowNodeDefinition {
  ...
  if (type === 'map')
    return { id, name, type, map: { source: '', key: '', join: '', subworkflow: { workflowId: '' } } };
...
export function WorkflowBuilder({
  ...
  function remove(id: string) {
    onChange({
      ...
      dependencies: (definition.dependencies ?? []).filter((edge) => edge.from !== id && edge.to !== id),
    });
  }
  ...
  function rename(node: WorkflowNodeDefinition, id: string) {
    ...
    onChange({
      ...definition,
      nodes: definition.nodes.map((current) => (current.id === node.id ? { ...current, id } : current)),
      dependencies: (definition.dependencies ?? []).map((edge) => ({
        ...edge,
        from: edge.from === node.id ? id : edge.from,
  ...
  function connect(connection: Connection) {
    if (
      ...
      definition.dependencies.some((edge) => edge.from === connection.source && edge.to === connection.target)
    )
      return;
    setEdges((current) => addEdge(connection, current));
    onChange({
      ...definition,
      dependencies: [...(definition.dependencies ?? []), { from: connection.source, to: connection.target }],
    });
  }
  ...
  return (
    <section className="workflow-builder" aria-label="Workflow builder">
      <div className="workflow-builder-header">
        <div>
          <span className="workflow-kicker">Visual builder</span>
          <h3>Workflow editor</h3>
        </div>
        <div className="workflow-tabs" role="tablist" aria-label="Workflow editor view">
          <Button role="tab" variant="muted" aria-selected={view === 'graph'} onClick={() => setView('graph')}>
            Editor
          </Button>
          <Button role="tab" variant="muted" aria-selected={view === 'yaml'} onClick={() => setView('yaml')}>
            YAML
          </Button>
        </div>
      </div>
        ...
        <div className="workflow-builder-layout">
          <div className="workflow-canvas" aria-label="Workflow canvas">
            <div className="workflow-add">
              <Button variant="accent" size="large" aria-expanded={addOpen} onClick={() => setAddOpen((open) => !open)}>
                + Add
              </Button>
              {addOpen && (
                <div role="menu" aria-label="Node types">
                  {nodeTypesList.map((type) => (
                    <Button key={type} variant="muted" role="menuitem" onClick={() => add(type)}>
                      {type}
                    </Button>
                  ))}
                </div>
...
function NodePanel({
  ...
  onUpdate: (node: WorkflowNodeDefinition) => void;
  onRename: (node: WorkflowNodeDefinition, id: string) => void;
  onDelete: (id: string) => void;
  onClose: () => void;
}) {
  if (!node) {
    function updateTrigger(index: number, patch: Partial<WorkflowDefinition['triggers'][number]>) {
      onDefinitionChange({
        ...definition,
        triggers: definition.triggers.map((trigger, current) =>
          current === index ? { ...trigger, ...patch } : trigger,
        ),
      });
    }

    function addTrigger() {
      const id = `trigger_${definition.triggers.length + 1}`;
      onDefinitionChange({ ...definition, triggers: [...definition.triggers, { id, type: 'manual' }] });
    }

    return (
      <aside className="workflow-node-panel" aria-label="Workflow properties">
        ...
        <div className="workflow-node-summary">
          ...
          <small>Triggers and schedule</small>
        </div>
        <label>
          ID
          <input
            value={definition.id}
            onChange={(event) => onDefinitionChange({ ...definition, id: event.target.value })}
          />
        </label>
        <label>
          Name
        ...
        <div className="workflow-trigger-editor">
          <h4>Triggers</h4>
          <Button variant="muted" onClick={addTrigger}>
            Add trigger
          </Button>
          {definition.triggers.map((trigger, index) => (
            <section key={trigger.id}>
              <label>
                ID
                <input value={trigger.id} onChange={(event) => updateTrigger(index, { id: event.target.value })} />
              </label>
              <label>
                Type
                <select
                  value={trigger.type}
                  onChange={(event) => updateTrigger(index, { type: event.target.value as typeof trigger.type })}
                >
              ...
              <label>
                ...
                <select
                  ...
                  onChange={(event) =>
                    updateTrigger(index, { overlap: event.target.value as 'skip' | 'queue' | 'parallel' })
                  }
                >
              ...
              {trigger.type === 'interval' && (
                <label>
                  ...
                  <input
                    ...
                    onChange={(event) =>
                      updateTrigger(index, { intervalSeconds: Number(event.target.value) || undefined })
                    }
                  />
                </label>
              )}
              {trigger.type === 'cron' && (
                <label>
                  ...
                  <input
                    value={trigger.cron ?? ''}
                    onChange={(event) => updateTrigger(index, { cron: event.target.value })}
                  />
                </label>
              )}
              <Button
                ...
                onClick={() =>
                  onDefinitionChange({
                    ...definition,
                    triggers: definition.triggers.filter((_, current) => current !== index),
                  })
                }
              >
                Remove trigger
  ...
  return (
    <aside className="workflow-node-panel" aria-label="Node properties">
      <div className="workflow-node-panel-title">
        <h3>Configure Node</h3>
        <button type="button" aria-label="Close node properties" onClick={onClose}>
          ×
        </button>
      </div>
      <div className="workflow-node-summary">
        ...
        <small>Workflow step configuration</small>
      </div>
      <label>
        ID<small>Lowercase letters, numbers, underscores, and hyphens only</small>
        <input value={node.id} onChange={(event) => onRename(node, event.target.value)} />
      </label>
      <label>
        Name
        <input value={node.name} onChange={(event) => onUpdate({ ...node, name: event.target.value })} />
      </label>
      <label>
        Type
        <select
          value={node.type}
          onChange={(event) => onUpdate(defaults(event.target.value as WorkflowNodeDefinition['type'], node.id))}
        >
          {nodeTypesList.map((type) => (
        ...
        <>
          ...
          <label>
            ...
            <input
              ...
              onChange={(event) => onUpdate({ ...node, command: event.target.value ? [event.target.value] : [] })}
            />
          </label>
        </>
      )}
      {node.type === 'agent' && (
        <>
          ...
          <label>
            Directory
            <input
              value={node.agent?.directory ?? ''}
              onChange={(event) => onUpdate({ ...node, agent: { ...node.agent!, directory: event.target.value } })}
            />
          </label>
          <label>
            Prompt
            <textarea
              ...
              onChange={(event) => onUpdate({ ...node, agent: { ...node.agent!, prompt: event.target.value } })}
            />
          </label>
          <label>
            Model
          ...
          <label>
            ...
            <input
              placeholder="high"
              value={node.agent?.reasoning ?? ''}
              onChange={(event) =>
                onUpdate({ ...node, agent: { ...node.agent!, reasoning: event.target.value || undefined } })
              }
            />
          </label>
          <label>
            ...
            <input
              placeholder="build"
              value={node.agent?.agent ?? ''}
              onChange={(event) =>
                onUpdate({ ...node, agent: { ...node.agent!, agent: event.target.value || undefined } })
              }
            />
          </label>
        </>
      ...
      {node.type === 'subworkflow' && (
        <>
          ...
          <label>
            Workflow ID
            <input
              value={node.subworkflow?.workflowId ?? ''}
              onChange={(event) => onUpdate({ ...node, subworkflow: { workflowId: event.target.value } })}
            />
          </label>
        </>
      )}
        ...
        <>
          ...
          <label>
            ...
            <input
              ...
              onChange={(event) => onUpdate({ ...node, map: { ...node.map!, source: event.target.value } })}
            />
          </label>
          <label>
            Item key
            <input
              value={node.map?.key ?? ''}
              onChange={(event) => onUpdate({ ...node, map: { ...node.map!, key: event.target.value } })}
            />
          </label>
          <label>
            Subworkflow ID
            <input
              value={node.map?.subworkflow.workflowId ?? ''}
              onChange={(event) =>
                onUpdate({ ...node, map: { ...node.map!, subworkflow: { workflowId: event.target.value } } })
              }
            />
          </label>
          <label>
            ...
            <input
              value={node.map?.join ?? ''}
              onChange={(event) => onUpdate({ ...node, map: { ...node.map!, join: event.target.value } })}
            />
          </label>
        </>
      )}
      {node.type === 'join' && (
        <>
          ...
          <label>
            Policy
            <select
              value={node.join?.policy ?? 'all-success'}
              onChange={(event) =>
                onUpdate({ ...node, join: { policy: event.target.value as NonNullable<typeof node.join>['policy'] } })
              }
            >
              <option value="all-success">all-success</option>
              <option value="always">always</option>
              <option value="minimum-success">minimum-success</option>
            </select>
          </label>
        </>
      )}
      <Button className="workflow-node-delete" variant="muted" onClick={() => onDelete(node.id)}>
        Delete node
      </Button>
    </aside>
  );
