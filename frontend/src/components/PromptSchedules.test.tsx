...
const scheduled = {
  id: 'ps_old', directory: '/repo', prompt: 'inspect me', state: 'scheduled' as const,
...
it('covers create, list, inspect, cancel, run-now, and linked-session lifecycle', async () => {
  ...
  expect(await screen.findByRole('link', { name: 'Open session' })).toHaveAttribute('href', '/session/session%201');
...
it('refreshes background execution state', async () => {
  ...
  apiMock.list
    ...
    .mockResolvedValueOnce([{ ...scheduled, state: 'completed', sessionId: 'background-session' }]);
  render(<MemoryRouter><PromptSchedules directory="/repo" /></MemoryRouter>);
  await act(async () => {});
  await act(async () => { vi.advanceTimersByTime(5000); });
  expect(screen.getByRole('link', { name: 'Open session' })).toHaveAttribute('href', '/session/background-session');
  vi.useRealTimers();
});
